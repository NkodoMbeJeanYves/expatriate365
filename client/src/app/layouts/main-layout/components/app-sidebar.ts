...
@Component({
  ...
  imports: [RouterLink, RouterLinkActive, TranslatePipe, DrawerModule, NgTemplateOutlet],
  template: `
    <ng-template #sidebarContent let-showLabels="showLabels">
      <!-- Logo -->
      <div class="flex items-center gap-3 px-4 py-5 border-b border-gray-200 dark:border-gray-700/50 flex-shrink-0">
        ...
        @for (group of visibleGroups(); track group.titleKey) {
          ...
          @for (item of group.items; track item.route) {
            <a [routerLink]="item.route"
               ...
               [title]="!showLabels ? (item.labelKey | translate) : ''">
      ...
      @if (store.user(); as user) {
        <div class="border-t border-gray-200 dark:border-gray-700/50 px-3 py-3 flex items-center gap-3 min-w-0 flex-shrink-0">
    ...
    <!-- Desktop sidebar -->
    <aside class="hidden lg:flex flex-col h-full bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800
                  transition-all duration-300 flex-shrink-0"
           [class.w-60]="!collapsed()" [class.w-16]="collapsed()">
      <ng-container [ngTemplateOutlet]="sidebarContent" [ngTemplateOutletContext]="{ showLabels: !collapsed() }" />
    </aside>

    <!-- Mobile drawer -->
    <p-drawer [visible]="mobileOpen()" (visibleChange)="mobileOpenChange.emit($event)"
              position="left" styleClass="!p-0 !w-72">
      <aside class="flex flex-col h-full bg-white dark:bg-gray-900">
        <ng-container [ngTemplateOutlet]="sidebarContent" [ngTemplateOutletContext]="{ showLabels: true }" />
      </aside>
    </p-drawer>
  `,
})
