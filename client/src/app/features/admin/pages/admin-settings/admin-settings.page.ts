    ...
    <div class="p-6 max-w-2xl flex flex-col gap-6">
      ...
      @if (loading()) {
        ...
        <form [formGroup]="form" (ngSubmit)="save()" class="flex flex-col gap-6">
          ...
          >
            ...
            <div class="flex flex-col gap-1">
              <div class="flex items-center gap-4">
                <div
                  class="w-16 h-16 rounded-lg bg-gray-100 border border-gray-200 overflow-hidden flex items-center justify-center flex-shrink-0"
                >
                  @if (logoPreview()) {
          ...
          >
            <h2 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">
              <i class="pi pi-building mr-2 text-emerald-600"></i
          ...
          >
            <h2 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">
              <i class="pi pi-wallet mr-2 text-amber-500"></i
              >{{ 'settings.currency_section' | translate }}
            </h2>
            <div class="flex flex-col gap-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              ...
              <div>
                ...
                <p class="text-xs text-gray-400 mt-1">
                  {{ 'settings.currency_symbol_hint' | translate }}
                </p>
              </div>
            </div>
          </div>
...
export class AdminSettingsPage implements OnInit {
  ...
  onLogoSelected(event: Event): void {
    ...
    this.http
      .post<{ file_url: string }>(`${this.config.apiUrl}/api/v1/upload?folder=branding`, formData)
      .subscribe({
        next: ({ file_url }) => {
