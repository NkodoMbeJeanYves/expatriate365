      ...
      <form [formGroup]="form" (ngSubmit)="onSubmit()" class="flex flex-col gap-4 p-2">
        <div class="flex flex-col gap-1">
          <label class="text-sm font-medium"
            >{{ 'community.post_title' | translate }} <span class="text-red-500">*</span></label
          >
          <input
            pInputText
            formControlName="title"
            [placeholder]="'community.post_title_placeholder' | translate"
          />
        </div>

        <div class="flex flex-col gap-1">
          <label class="text-sm font-medium"
            >{{ 'community.post_content' | translate }} <span class="text-red-500">*</span></label
          >
          <textarea
            pTextarea
            ...
            rows="10"
            class="w-full"
            [placeholder]="'community.post_content_placeholder' | translate"
          ></textarea>
        </div>
        ...
        <div class="flex flex-col gap-2">
          ...
          <div class="flex gap-2">
            <label
              class="flex items-center gap-2 px-3 py-2 border border-gray-200 rounded-lg cursor-pointer hover:border-emerald-400 text-sm transition-colors"
            >
          ...
          @if (uploading()) {
            <p class="text-sm text-gray-400">
              <i class="pi pi-spin pi-spinner mr-1"></i>{{ 'community.uploading' | translate }}
            </p>
          }
        ...
        <div class="flex justify-end gap-2 pt-2">
          ...
          <p-button
            type="submit"
            [label]="
              editPost ? ('common.save' | translate) : ('community.submit_draft' | translate)
            "
            [loading]="saving()"
...
export class PostFormDrawerComponent {
  ...
  readonly pendingAttachments = signal<
    {
      file_url: string;
      file_name: string;
      mime_type: string;
  ...
  onFileSelected(event: Event, type: 'photo' | 'document'): void {
    ...
    this.http
      .post<{ file_url: string; file_name: string; file_size_bytes: number; mime_type: string }>(
        `${this.config.apiUrl}/api/v1/upload?folder=${folder}`,
        fd,
      )
      .subscribe({
        next: (r) => {
